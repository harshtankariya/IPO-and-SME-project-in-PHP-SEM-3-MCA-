...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if ($stmt->execute()) {
            if ($stmt->rowCount() > 0) {
                echo "<script>alert('IPO record updated successfully!'); window.location.href='demo_display.php';</script>";
            } else {
                echo "<script>alert('No changes made. Record already up-to-date.'); window.location.href='demo_display.php';</script>";
            }
        } else {
